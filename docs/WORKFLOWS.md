...
Wizard Calendar **Connect** runs Google OAuth with `returnTo=/onboarding/{tenantSlug}?step=calendar`; success/error returns to that step (`calendar=connected` / `calendar=error`). Login honors safe `returnTo` when session is required mid-flow.
...
Phase D1: **Wizard AI (tagline + about)** — authenticated `POST /api/admin/tenants/[tenantSlug]/wizard-ai/propose` returns proposals only; ✨ opens a proposal panel (Use This / Edit / Try Again / Cancel). Form state updates on accept; persistence remains wizard `saveProgress()` → tenant PATCH. Principle: **AI proposes; human accepts; system remains truth** (`docs/DECISIONS.md` **D-030**).

Phase D2: **Wizard Services AI** — same propose route with `action: "services"`. Services describe **what the business does**; Customer Experience step owns **how the receptionist helps**. Use This **replaces** the services textarea (no append). Saved `services_offered` flows to hosted website service cards when website-specific services are not configured (`TenantWebsite.getServices()`). **D-031**.

Phase D1.1: Wizard About/tagline prompts use **on-behalf-of-business** voice (we/us/our for About); owner About textarea is primary factual source (`ownerProvidedBusinessDescription`). Customer Help step heading/options clarify AI receptionist behavior; `bookingType` values unchanged.
