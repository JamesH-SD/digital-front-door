...
Phase D1: **Wizard AI (tagline + about)** — authenticated `POST /api/admin/tenants/[tenantSlug]/wizard-ai/propose` returns proposals only; ✨ opens a proposal panel (Use This / Edit / Try Again / Cancel). Form state updates on accept; persistence remains wizard `saveProgress()` → tenant PATCH. Services ✨ remains template placeholder until D2. Principle: **AI proposes; human accepts; system remains truth** (`docs/DECISIONS.md` **D-030**).
